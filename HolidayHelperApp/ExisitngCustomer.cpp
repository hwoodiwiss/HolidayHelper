#include "ExisitngCustomer.h"

ExistingCustomer::ExistingCustomer()
{
}

string ExistingCustomer::Execute(string prevPage, shared_ptr<DataManager> appDataManager, shared_ptr<User>& appUser)
{
	string Username;
	string Password;
	int count = 0;
	...
	while (count < 3)
	{
		...
		if (foundUser != nullptr && foundUser->GetPassword() == Password)
		{
			...
			if (foundUser->GetUserType() == UT_ADMIN)
			{
				return "AdminMain";
			}
			return "Main";
